...
<div class="hidden px-4 md:flex-1 md:flex md:items-middle md:justify-between md:space-x-12">
    <div v-if="showBranding=='true'" class="flex1 items-center object-none object-center">
        <a href="#">
            ...
            <nuxt-link tag="img" to="/" alt="Logo" width="200" class="h-8 w-auto sm:h-10" 
             src="https://res.cloudinary.com/nathansweb/image/upload/v1614398508/xyz/Z_Logo_Alone_ditioa.svg"
            >
            </nuxt-link>
        </a>
    </div>
        ...
        <nav class="flex space-x-10 m-4">
            <div v-for="(menu,index) in menuitems.menus" :key="menu.link">
                <div class="relative" v-if="menu.children">
                  ...
                  transition ease-in-out duration-150" :class="textColor!='green' ? 'text-${textColor}' : 'text-yellow-500  hover:text-yellow-600  focus:text-yellow-600'">
                        <span>{{menu.title}}</span>
                        <!-- Item active: "text-gray-600", Item inactive: "text-gray-400" -->
                        <svg class="transition ease-in-out duration-150" viewBox="0 0 20 20" fill="currentColor" :class="textColor!='green' ? 'text-${textColor} h-5 w-5 group-hover:text-white group-focus:text-white' : 'text-yellow-400 h-5 w-5 group-hover:text-yellow-500 group-focus:text-yellow-500'">
                ...
                <div v-else>
                    <router-link v-bind:to="menu.link" class="uppercase text-base leading-6 font-medium  focus:outline-none transition ease-in-out duration-150 nav-link"  :class="textColor!='green' ? 'text-${textColor}' : 'text-yellow-500  hover:text-yellow-600  focus:text-yellow-600' ">{{menu.title}}</router-link>
                    <!-- <a :href="menu.link" 
                    @click="toggle(index)"
                    class="uppercase text-base leading-6 font-medium  focus:outline-none transition ease-in-out duration-150"  :class="textColor!='green' ? 'text-${textColor}' : 'text-yellow-500  hover:text-yellow-600  focus:text-yellow-600' ">
                        {{menu.title}}
                    </a> -->
                </div>
            </div>
    ...
    <div class="flex items-center space-x-8 -mt-2">
        ...
        <span v-if="showActionButtons=='true' && menu.hidden==false" v-for="menu in menuitems.actions" :key="menu.link" class="inline-flex rounded-md shadow-sm">
            <a href="#" class="uppercase inline-flex items-center justify-center px-4 py-2 border border-transparent text-base leading-6 font-medium rounded-md text-white bg-yellow-600 hover:bg-yellow-500 focus:outline-none focus:border-green-700 focus:shadow-outline-green active:bg-yellow-700 transition ease-in-out duration-150">
                {{menu.title}}
            </a>
        </span>
    ...
    <div class="absolute top-0 inset-x-0 p-2 transition transform origin-top md:hidden">
        <div class="rounded-lg shadow-md bg-white ring-1 ring-black ring-opacity-5 overflow-hidden">
            <div class="px-5 pt-4 flex items-center justify-between">
                <div>
                    <nuxt-link tag="img" to="/" alt="Logo" width="200" class="pt-2" src="https://res.cloudinary.com/nathansweb/image/upload/v1631980578/logos/logo-mark-flower-indigo_pmykvr.svg"
                    >
                    </nuxt-link>
                </div>
                <div class="-mr-2">
            ...
            <div class="pt-5 pb-6">
                <div class="px-2 space-y-1">
                    <a v-for="(menu,index) in menuitems.menus" :key="menu.link" href="#" class="block px-3 py-2 rounded-md text-base font-medium text-yellow-900 hover:bg-yellow-50">
                        {{menu.title}}
                    </a>
                </div>
            </div>
